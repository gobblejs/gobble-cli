...
function indent ( str, indentAmount ) {
	...
	numColumns = ( process.stdout.columns || process.stderr.columns || 80 ) - indentAmount;
...
function clearLineAfter ( offset, text ) {
	...
	cols = ( process.stdout.columns || process.stderr.columns || 80 ) - ( offset + text.length );
...
messages = {
	BUILD_INVALIDATED: function ( x ) {
		return f( '\n\n\n\nbuild invalidated (%s). restarting', summariseChanges( x.changes ) );
	},
	...
	BUILD_COMPLETE: function ( x ) {
		...
		if ( x.port ) result += 'server listening on port ' + chalk.bold( x.port ) + '\n';
...
errorHandlers = {
	...
	TRANSFORMATION_FAILED: function ( err ) {
		...
		// 0.7.11 includes inputdir and outputdir on error object
		if ( err.inputdir ) {
			console.error( 'input:  %s', err.inputdir );
			console.error( 'output: %s', err.outputdir );
		}
...
logger = {
	...
	error: function ( err, cb ) {
		...
		if ( err.name === 'GobbleError' && ( errorHandler = errorHandlers[ err.code ] ) ) {
			...
			error( err instanceof Error ? err.message : util.format( err ) );

			console.log( chalk.red( '>>>' ) );
			console.log( chalk.grey( err.stack || ( new Error().stack ) ) );
			console.log( chalk.red( '<<<' ) );
